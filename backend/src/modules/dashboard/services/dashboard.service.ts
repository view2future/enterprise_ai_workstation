...
export class DashboardService {
  ...
  async getStats(timeRange: string = 'all', userEnv: string = 'PROD') {
    const where: any = { status: 'active', env: userEnv };
  ...
  async getTechRadarData(userEnv: string = 'PROD') {
    ...
    const [ernieModels, paddleLevels, stages] = await Promise.all([
      this.prisma.enterprise.groupBy({ by: ['ernieModelType'], where, _count: { _all: true } }),
      this.prisma.enterprise.groupBy({ by: ['paddleUsageLevel'], where, _count: { _all: true } }),
      this.prisma.enterprise.groupBy({ by: ['aiImplementationStage'], where, _count: { _all: true } })
    ]);
    return { ernieModels, paddleLevels, stages };
  }
  ...
  async getMapData(userEnv: string = 'PROD') {
    return this.prisma.enterprise.findMany({
      where: { status: 'active', env: userEnv },
      take: 1000
    });
  }
}
